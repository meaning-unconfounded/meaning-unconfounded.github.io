 ...
 * Listening Room, Key System (40 interactive keys), NotebookLM demo
 */
...
(function() {
  ...
  /* ── KEY SYSTEM (40 Interactive Keys) ──────────────────── */
  (function() {
    // ----- Configuration -----
    var KEY_IDS = [];
    for (var i = 1; i <= 40; i++) {
      KEY_IDS.push(i.toString().padStart(2, '0'));
    }

    // ----- Audio (Web Audio API, separate context) -----
    var audioCtx = null;
    ...
    function playKeySound(color) {
      ...
      var freq = 440;
      if (color === 'pink') freq = 880;
      else if (color === 'green') freq = 660;
      else if (color === 'gold') freq = 440;
      else freq = 660;
    ...
    // ----- Key reveal / ghost logic -----
    var activeKeys = {}; // keyId -> DOM element

    // Find all key elements
    document.querySelectorAll('.key-surprise').forEach(function(el) {
      var kid = el.dataset.keyId;
      if (kid) activeKeys[kid] = el;
    });

    // ----- Intersection Observer for keys -----
    var keyObserver = new IntersectionObserver(function(entries) {
      entries.forEach(function(entry) {
        ...
        if (entry.isIntersecting) {
          // Reveal if not seen and not already revealed
          if (!el.classList.contains('slide-in') && !el.classList.contains('ghost')) {
            // Check if this key should appear (not seen, or if seen but we still show it)
            // We show it even if seen, but we don't play sound again
          ...
          // Scrolled past – ghost it if not engaged
    ...
    // Observe all keys
    document.querySelectorAll('.key-surprise').forEach(function(el) {
      keyObserver.observe(el);
    });

    // ----- Click to engage / expand (remove ghost, show checkmark) -----
    document.querySelectorAll('.key-card').forEach(function(card) {
      card.addEventListener('click', function(e) {
        ...
        if (!container.querySelector('.key-checkmark')) {
          var cm = document.createElement('span');
          cm.className = 'key-checkmark';
          cm.textContent = '✓';
    ...
    // ----- Seed hover to re-expand ghost -----
    document.querySelectorAll('.key-seed').forEach(function(seed) {
      seed.addEventListener('mouseenter', function() {
        var container = this.closest('.key-surprise');
        if (container && container.classList.contains('ghost')) {
          container.classList.remove('ghost');
          container.style.opacity = '1';
        }
      });
      seed.addEventListener('mouseleave', function() {
        var container = this.closest('.key-surprise');
        if (container && !container.classList.contains('engaged')) {
          // Re-ghost after a short delay if not engaged
          setTimeout(function() {
            if (!container.classList.contains('engaged')) {
    ...
    // ----- Tracker toggle (click to expand detail) -----
    ...
    // ----- Re-check keys on scroll (for ghost re-entry) -----
    // On scroll-back-up, if a ghosted key comes back into view, remove ghost
    // This is handled by the observer: when it becomes intersecting again, it will remove ghost.
    // But we need to ensure ghost is removed when re-entering viewport.
    // The observer will trigger again when intersecting; we can add logic to un-ghost.
    // We'll add a separate observer or modify the existing one.
    // Simpler: when observer sees it intersecting, if it's ghost and not engaged, un-ghost.
    // We'll override the observer callback to handle this.
    // Since we already have the observer, we'll modify its callback.
    // But we need to re-declare it. We'll replace the observer with a new one.
    ...
    var newObserver = new IntersectionObserver(function(entries) {
      entries.forEach(function(entry) {
        ...
        if (entry.isIntersecting) {
          // If it's a ghost, un-ghost it (without playing sound again)
          ...
          // Scrolled past – ghost if not engaged
          if (el.classList.contains('slide-in') && !el.classList.contains('engaged')) {
            el.classList.add('ghost');
          }
        }
    ...
    // ----- Clean up: ensure keys are properly positioned after load -----
    window.addEventListener('load', function() {
      // Force a re-check of all keys
      document.querySelectorAll('.key-surprise').forEach(function(el) {
        var rect = el.getBoundingClientRect();
        var isVisible = (rect.top < window.innerHeight && rect.bottom > 0);
        if (isVisible && !el.classList.contains('slide-in')) {
