 ...
 * Listening Room, Key System (Corner Badge + Drawer), NotebookLM demo
 */

(function() {
  "use strict";

  // ── inject styles for the new key system ─────────────────────
  (function injectKeyStyles() {
    const styles = `
      ...
      .drawer-item.expanded .item-insight {
        display: block;
      }
      .drawer-item .item-status {
        font-size: 0.6rem;
        color: var(--muted);
        margin-top: 0.15rem;
        font-family: var(--sans);
      }
      .drawer-item.unread .item-title::after {
        content: " •";
        color: var(--key-pink);
        font-weight: 700;
      }
      ...
      .drawer-item .item-actions button {
        background: none;
        border: none;
        font-size: 0.8rem;
        cursor: pointer;
      ...
      .drawer-item .item-actions button:hover {
        background: var(--surface-2);
        color: var(--gold);
      }
      .drawer-empty {
  ...
  /* ── KEY SYSTEM — Corner Badge + Drawer ───────────────── */
  (function() {
    // ----- Key data (hardcoded from existing 40 keys) -----
    const KEY_DATA = [
      { id: '01', title: 'M = Material', insight: 'Close your lips and hum. That vibration is not a sound—it is a substance. You just executed material existence through your own anatomy.', emoji: '👄', color: 'pink', section: 'mu-ch1' },
      { id: '02', title: 'SPREAD vs SPRAY', insight: 'SPREAD ends in D: the energy arrives and stops. SPRAY ends in Y: the energy keeps going. One consonant changes the timeline of the action.', emoji: '⏱️', color: 'green', section: 'mu-intro' },
      { id: '03', title: 'The Silent K', insight: 'Say KNOW and NOW. Your tongue still rises for the silent K. The meaning is present but invisible—held within a boundary you cannot hear.', emoji: '🔇', color: 'yellow', section: 'mu-ch9' },
      { id: '04', title: 'T vs R endings', insight: 'Words ending in T are sealed in the past: thought, met, went. Words ending in R are still radiating: fire, power, star.', emoji: '🔥/🔒', color: 'pink', section: 'mu-ch8' },
      { id: '05', title: 'The Verb Hides the Noun', insight: '"I am depressed" hides the thing. "The depression in me" makes it visible and fixable. Grammar is the first therapy.', emoji: '🎭', color: 'green', section: 'mu-ch13' },
      { id: '06', title: 'Four Cows', insight: 'The word "cow" is a human agreement. The number four is true everywhere. Math is objective; physical objects are the description.', emoji: '🐄', color: 'yellow', section: 'mu-ch5' },
      { id: '07', title: 'Gravity = Expansion', insight: 'You are not falling to Earth. The Earth is expanding up to meet you. Gravity is expansion, not attraction.', emoji: '🌍', color: 'pink', section: 'mu-ch16' },
      { id: '08', title: 'YHWH = Consciousness Iteration', insight: 'YHWH decodes to: Act, Breathe, Connect, Breathe. The oldest unspeakable word is the operating cycle of consciousness.', emoji: '✡️', color: 'green', section: 'mu-ch7' },
      { id: '09', title: 'N = The Sound of Time', insight: 'N is the only sound that lets air flow through your nose without stopping. The nasal cavity is the body\'s time-keeping organ.', emoji: '⏳', color: 'yellow', section: 'mu-ch8' },
      { id: '10', title: 'Body as Pronunciation', insight: 'TONGUE, THUMB, TOE, THOUGHT — all begin with T. All are points where the body contacts reality.', emoji: '🫀', color: 'pink', section: 'mu-ch16' },
      { id: '11', title: 'Questions contain W/H', insight: 'WHO, WHAT, WHERE, WHEN, WHY, HOW — all contain W (Connect) or H (Breath). Questions are connecting operations.', emoji: '❓', color: 'green', section: 'mu-ch3' },
      { id: '12', title: 'Prepositions as Geometry', insight: 'OF = flowing source. FOR = flowing radiance toward. BY = bounded deed. Prepositions are geometric relationship operators.', emoji: '📐', color: 'yellow', section: 'mu-ch14' },
      { id: '13', title: 'Prefixes as Steering', insight: 'RE- sends radiance back to source. EX- bursts from containment. Prefixes are steering commands.', emoji: '🧭', color: 'pink', section: 'mu-ch12' },
      { id: '14', title: 'Silent Consonant Proof', insight: 'Only stops (B, P, T, D, K, G) can go silent. You cannot suppress a continuous sound without killing the word.', emoji: '🧪', color: 'green', section: 'mu-ch9' },
      { id: '15', title: 'POSTULATE Self-Describes', insight: 'POSTULATE = Project + Spread + Mark + Lateral + Mark. A postulate is projected outward, spreads, marks, flows laterally, and marks again.', emoji: '🔄', color: 'yellow', section: 'mu-ch10' },
      { id: '16', title: 'CONSCIOUSNESS Self-Describes', insight: 'CONSCIOUSNESS = Contain + Continue + Spread + Contain + Spread + Continue + Spread + Spread. A field that contains its own continuation and spread.', emoji: '🪞', color: 'pink', section: 'mu-ch10' },
      { id: '17', title: 'MASS = Frozen Conflict', insight: 'MASS = Material + Spread + Spread. Frozen conflict is literal mass. Your psychological baggage has physical weight.', emoji: '⚖️', color: 'green', section: 'mu-ch16' },
      { id: '18', title: 'Emotions as Radiating', insight: 'ANGER = Continue + Grow + Radiate. FEAR = Flow + Radiate. Emotions are the radiating exhaust of frozen postulate conflict.', emoji: '😤', color: 'yellow', section: 'mu-ch16' },
      { id: '19', title: 'The Body Map', insight: 'Raised shoulders = "Must Not Be Known." Jaw tension = two postulates colliding. The body is the map of frozen conflict.', emoji: '🗺️', color: 'pink', section: 'mu-ch16' },
      { id: '20', title: 'Map vs Mechanism', insight: 'Describing the architecture of a locked room does not open the door. Understanding describes the map; change requires operating the mechanism.', emoji: '🗺️/🔧', color: 'green', section: 'mu-interlude' },
      { id: '21', title: 'The Hidden Twin', insight: 'A politician must be seen as a leader. Hidden beneath: must never be seen as ordinary. The visible postulate drives the game; the hidden twin locks the trap.', emoji: '👥', color: 'yellow', section: 'mu-ch15' },
      { id: '22', title: 'Frozen Postulates', insight: 'Frozen conflict has four characteristics: Identification, Motionlessness, Timelessness, Mass. This is the physics of trauma, codified.', emoji: '🧊', color: 'pink', section: 'mu-ch16' },
      { id: '23', title: 'Existence Cannot Not Exist', insight: 'Absolute nothing is a mathematical impossibility. Zero requires a framework; the framework cannot be absent.', emoji: '∞', color: 'green', section: 'mu-ch6' },
      { id: '24', title: 'Math is the Only Objective', insight: 'Mathematics is the only objective phenomenon. Physical objects are subjective descriptions. We have had it backward for 2,400 years.', emoji: '📐', color: 'yellow', section: 'mu-ch5' },
      { id: '25', title: 'The Universe Was Written', insight: 'The universe was not created from nothing. It was written. The Word is the meaning; the meaning is the substance.', emoji: '✍️', color: 'pink', section: 'mu-ch27' },
      { id: '26', title: 'ALL CAPS = Casting', insight: 'ALL CAPS seals the entity — rigid, bounded, dead. Cursive is the counter‑spell — continuous, living, unbroken.', emoji: '🔤', color: 'green', section: 'mu-ch30' },
      { id: '27', title: 'The Space Bar = Breath', insight: 'The space bar is the Lateral (L). It inserts breath between words. Without it, words collapse into frozen mass.', emoji: '␣', color: 'yellow', section: 'mu-ch30' },
      { id: '28', title: 'Punctuation as Operators', insight: 'Period = T (Mark). Comma = L (Lateral flow). Question mark = W+T (Connect+Mark). Punctuation is algorithmic.', emoji: '❗', color: 'pink', section: 'mu-ch30' },
      { id: '29', title: 'THE vs A', insight: 'THE = specifically marked, high importance. A = unmarked, general vessel. Articles are specificity instructions.', emoji: '🎯', color: 'green', section: 'mu-ch12' },
      { id: '30', title: 'Vowels as Gravity', insight: 'Vowels are gravity vectors. Consonants are the code. The code executes within the directional field of the vowel.', emoji: '🌌', color: 'yellow', section: 'mu-app-o' },
      { id: '31', title: 'AI Needs Architecture', insight: 'Current AI processes language statistically. If the consonant code is real, meaning is architectural, not statistical.', emoji: '🤖', color: 'pink', section: 'mu-ch23' },
      { id: '32', title: 'Legal Double Negatives', insight: '"Shall not without" hides the trap in plain sight. Legal documents use double negatives as linguistic Exclusion Anchors.', emoji: '⚖️', color: 'green', section: 'mu-ch19' },
      { id: '33', title: 'Democracy = Literacy', insight: 'A population that can decode headlines cannot be manipulated through linguistic freezing. Literacy is immunity.', emoji: '🗳️', color: 'yellow', section: 'mu-ch26' },
      { id: '34', title: '7‑Level Curriculum', insight: 'From consonant secrets to full stack: age 8 to 13. Children learning language as code, not memorization.', emoji: '🎓', color: 'pink', section: 'mu-ch22' },
      { id: '35', title: 'The 9‑Layer Genesis Stack', insight: 'Life → Postulates → Numbers → Logic → Logicism → Pictorial → Written Numbers → Letters → Programming. Reality is nested code.', emoji: '🏗️', color: 'green', section: 'mu-prologue' },
      { id: '36', title: 'The 1‑9 Matrix', insight: '1=Point, 2=Line, 3=Triangle... 9=Resolution. The alphabet is a creation sequence. Pythagorean geometry mapped to letter production.', emoji: '🔢', color: 'yellow', section: 'mu-app-p' },
      { id: '37', title: 'Master Numbers', insight: 'Master Numbers (11, 22, 33) are recursive loops. Normal numbers create entropy; Master Numbers create coherency.', emoji: '♾️', color: 'pink', section: 'mu-app-p' },
      { id: '38', title: 'The Seven Lenses', insight: 'Symmetry, Completeness, Scale, Inversion, Process, Hidden Anchor, Missing Corner — seven lenses for examining any structure.', emoji: '🔍', color: 'green', section: 'mu-app-h' },
      { id: '39', title: 'N‑Position Algorithm', insight: 'N at start: negate or initiate. N in middle: continue. N at end: arrive. Same letter, four functions. Position is meaning.', emoji: '🔄', color: 'yellow', section: 'mu-app-b' },
      { id: '40', title: 'The Voicing Switch', insight: 'P/B, T/D, K/G, F/V, S/Z, CH/J — your vocal cords are a physical switch. Off = released/sharp. On = held/continuing.', emoji: '🔘', color: 'pink', section: 'mu-ch11' }
    ];

    // ----- State management -----
    const STORAGE_KEY = 'wb_keys';
    let state = {
      unread: [],   // array of key IDs that are unread
      read: [],     // array of key IDs that have been read
      saved: []     // array of key IDs that are bookmarked
    };
    ...
    function loadState() {
      try {
        ...
        if (raw) {
          ...
          // Ensure all keys are in either unread or read (or saved)
          // For backwards compatibility, if a key is not in either, add to unread.
          const allIds = KEY_DATA.map(k => k.id);
          allIds.forEach(id => {
            if (!state.unread.includes(id) && !state.read.includes(id)) {
              // If it's not in state at all, treat as unread
              state.unread.push(id);
            }
          });
          // Remove duplicates
          state.unread = [...new Set(state.unread)];
          state.read = [...new Set(state.read)];
          state.saved = [...new Set(state.saved)];
          // Ensure saved keys are also in read or unread (they should be)
          ...
          // First time: all keys are unread
          state.unread = KEY_DATA.map(k => k.id);
          state.read = [];
          state.saved = [];
          persistState();
        }
    ...
    function markRead(id) {
      ...
      const idx = state.unread.indexOf(id);
      if (idx !== -1) state.unread.splice(idx, 1);
      persistState();
      updateBadge();
    }
    ...
    function markUnread(id) {
      ...
      if (idx !== -1) state.read.splice(idx, 1);
      persistState();
      updateBadge();
    }
    ...
    function toggleSaved(id) {
      ...
      if (idx !== -1) {
        ...
        if (!state.read.includes(id)) {
          state.read.push(id);
          const uidx = state.unread.indexOf(id);
          if (uidx !== -1) state.unread.splice(uidx, 1);
        }
      }
      persistState();
      updateBadge();
    ...
    function isUnread(id) { return state.unread.includes(id); }
    function isSaved(id) { return state.saved.includes(id); }

    // ----- UI Elements -----
    let badge, drawer, overlay, listEl, tabs, closeBtn, markAllBtn;

    function createUI() {
      // Badge
      badge = document.createElement('div');
      badge.id = 'keyBadge';
      badge.innerHTML = `<span>💎</span><span class="badge-count" id="badgeCount"></span>`;
      document.body.appendChild(badge);
      badge.addEventListener('click', toggleDrawer);

      // Overlay
      overlay = document.createElement('div');
      overlay.id = 'keyDrawerOverlay';
      ...
      drawer.innerHTML = `
        <div class="drawer-header">
          <span class="drawer-title">💎 Insights</span>
          <button class="drawer-close" id="drawerClose">✕</button>
        </div>
        <div class="drawer-tabs">
          <span data-tab="unread" class="active">Unread</span>
          <span data-tab="all">All</span>
          <span data-tab="saved">Saved</span>
        </div>
      ...
      // Tabs
      tabs = drawer.querySelectorAll('.drawer-tabs span');
      tabs.forEach(tab => {
    ...
    function updateBadge() {
      ...
      if (badgeCount) {
        badgeCount.textContent = count > 0 ? count : '';
      }
      if (badge) {
        if (count > 0) {
    ...
    function renderDrawer() {
      ...
      const activeTab = document.querySelector('.drawer-tabs .active');
      let tab = 'unread';
      if (activeTab) tab = activeTab.dataset.tab;

      let filteredIds = [];
      if (tab === 'unread') {
        filteredIds = state.unread.slice();
      } else if (tab === 'saved') {
        filteredIds = state.saved.slice();
      } else { // 'all'
        filteredIds = KEY_DATA.map(k => k.id);
      }

      // Sort: unread first, then by id
      filteredIds.sort((a, b) => {
        const aUnread = state.unread.includes(a);
        const bUnread = state.unread.includes(b);
      ...
      if (filteredIds.length === 0) {
        list.innerHTML = `<div class="drawer-empty">No insights in this view.</div>`;
        updateFooterCount(0);
        return;
      }
      ...
      filteredIds.forEach(id => {
        const key = KEY_DATA.find(k => k.id === id);
        if (!key) return;
        const read = isRead(id);
        const unread = isUnread(id);
        const saved = isSaved(id);
        const emoji = key.emoji || '💎';
        const title = key.title || 'Untitled';
        const insight = key.insight || '';

        html += `
          <div class="drawer-item ${unread ? 'unread' : ''}" data-id="${id}">
            <div class="item-icon">${emoji}</div>
            <div class="item-content">
              <div class="item-title">${title}</div>
              <div class="item-insight">${insight}</div>
              <div class="item-status">${read ? '✓ Read' : '📌 New'}</div>
            </div>
            <div class="item-actions">
              <button class="save-btn" title="Save/Bookmark">${saved ? '★' : '☆'}</button>
              ${read ? `<button class="mark-unread-btn" title="Mark as unread">↩</button>` : ''}
            </div>
          </div>
        `;
      });
      ...
      list.innerHTML = html;
      updateFooterCount(filteredIds.length);

      // Attach event listeners
      list.querySelectorAll('.drawer-item').forEach(item => {
        const id = item.dataset.id;
        // Click to expand/collapse insight
        item.querySelector('.item-content').addEventListener('click', function(e) {
          e.stopPropagation();
          item.classList.toggle('expanded');
          // If it was unread, mark as read when expanded
          if (isUnread(id) && !isRead(id)) {
            markRead(id);
            // Update status text
            ...
            if (actions && !actions.querySelector('.mark-unread-btn')) {
              const btn = document.createElement('button');
              btn.className = 'mark-unread-btn';
        ...
        // Mark unread button (only if read)
    ...
    function updateFooterCount(count) {
      ...
      if (el) {
        const total = KEY_DATA.length;
        const unreadCount = state.unread.length;
        el.textContent = `${count} shown · ${unreadCount} unread`;
      }
    }

    // ----- Section Observer for auto‑marking read on scroll -----
    function setupSectionObserver() {
      const sectionMap = {};
      KEY_DATA.forEach(k => {
        if (!sectionMap[k.section]) sectionMap[k.section] = [];
        sectionMap[k.section].push(k.id);
      });

      const observer = new IntersectionObserver(function(entries) {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            const ids = sectionMap[entry.target.id];
            if (ids) {
              ids.forEach(id => {
                if (isUnread(id)) {
                  markRead(id);
        ...
        rootMargin: '0px 0px -50% 0px', // mark when section is mostly in view
        threshold: 0.3
      });

      // Observe all sections that have keys
    ...
    // ----- Hide old key cards (if any) -----
    function hideOldKeyCards() {
      document.querySelectorAll('.key-surprise').forEach(el => {
    ...
    function init() {
      ...
      // Also update badge when scrolling (in case user reads without opening drawer)
      window.addEventListener('scroll', function() {
        // Badge update is already handled via observer, but we can force a re-render if needed
        // The observer will mark read, which updates badge via markRead.
      });
    }
