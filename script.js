...
(function() {
  ...
  (function() {
    // ----- Mapping: keyId -> sectionId (mapped to valid main anchors) -----
    var keyMap = {
      '01': 'mu-ch1',
      '02': 'mu-intro',
      '03': 'mu-ch9',
    ...
    function playKeySound(color) {
      ...
      // Create a soft, muffled chime instead of a harsh ding
      var osc = ctx.createOscillator();
      var gain = ctx.createGain();
      var filter = ctx.createBiquadFilter();
      
      osc.type = 'sine'; // Sine wave is the smoothest
      filter.type = 'lowpass';
      filter.frequency.value = 1200; // Cut harsh high frequencies
      
      var freq = 440;
      if (color === 'pink') freq = 523.25; // C5
      else if (color === 'green') freq = 392.00; // G4
      else if (color === 'gold') freq = 440.00; // A4
      
      osc.frequency.value = freq;
      
      // Smooth attack and decay envelope
      gain.gain.setValueAtTime(0, ctx.currentTime);
      gain.gain.linearRampToValueAtTime(0.08, ctx.currentTime + 0.02); // Very low volume
      gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 0.5); // Smooth fade out
    ...
    var QUEUE_DELAY = 1200; // 1.2 seconds between reveals

    function processQueue() {
      if (isRevealing || revealQueue.length === 0) return;
      isRevealing = true;
      
      // Immediately hide all other visible, non-engaged keys to prevent stacking
      document.querySelectorAll('.key-surprise.slide-in').forEach(function(el) {
    ...
    function queueReveal(kid) {
      // Only queue if not already in queue and not already revealed/engaged
    ...
    document.querySelectorAll('.key-card').forEach(function(card) {
      card.addEventListener('click', function(e) {
        ...
        // Mark as engaged so it doesn't pop back up
        container.classList.add('engaged');
        
        // Slide it out and hide it
