...
(function() {
  ...
  // ── Map each key to the section it should appear with ──
  var topicMap = {
    '01': 'mu-ch1',      // M‑demonstration
    '02': 'mu-ch9',      // Silent K
    '03': 'mu-ch8',      // D vs Y (Position)
    '04': 'mu-ch13',     // Math objectivity / Verb Illusion
    '07': 'mu-ch7',      // YHWH
    '06': 'mu-ch15',     // Hidden twin
    '09': 'mu-ch16',     // T‑extremities (Body as Pronunciation)
    '10': 'mu-ch16',     // Frozen conflict is mass (same chapter)
    '21': 'mu-ch4',      // Existence cannot not exist
    '30': 'mu-prologue', // 9‑layer stack
    '31': 'mu-ch31',     // Cosmology / Tube
    '32': 'mu-ch31',     // Egg
    '33': 'mu-ch31',     // Light
    '34': 'mu-ch31',     // Eclipse
    '35': 'mu-ch31',     // Silence
  ...
  keys.forEach(function(key, index) {
    ...
    // Click/hover/touch to dismiss (unchanged)
  ...
  // ── NEW: Observe the topic sections instead of sentinels ──
  var topicObserver = new IntersectionObserver(function(entries) {
    entries.forEach(function(entry) {
      var targetId = entry.target.id;
      // Find which key(s) map to this section
      var matchingKeys = [];
      for (var keyId in topicMap) {
        if (topicMap[keyId] === targetId) {
          matchingKeys.push(keyId);
        }
      }

      matchingKeys.forEach(function(kid) {
        var key = document.querySelector('.key-surprise[data-key-id="' + kid + '"]');
        if (!key) return;

        if (entry.isIntersecting) {
          // Section is visible – show the key
          showKey(key, kid);
        } else {
          // Section is hidden – hide the key
          hideKey(key);
        }
      });
    });
  }, {
    rootMargin: '-10% 0px -30% 0px', // Show when section is roughly in the middle
    threshold: 0
  });

  // Observe each target section
  for (var kid in topicMap) {
    var sectionId = topicMap[kid];
    var sectionEl = document.getElementById(sectionId);
    if (sectionEl) {
      topicObserver.observe(sectionEl);
    }
  }

  // ── Tracker click toggle (unchanged) ──
